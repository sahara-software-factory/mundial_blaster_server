...
app.delete('/api/campaigns/:id', authOrSecret, requireLicense, async (req, res) => {
  try {
    ...
    const where = { id }
    if (userId) {
      where.OR = [{ owner_id: userId }, { owner_id: null }]
    }
    
    const campaign = await prisma.campaigns.findFirst({ where })
    if (!campaign) return res.status(404).json({ error: 'Campaña no encontrada' })
...
app.get('/api/bases-datos', authOrSecret, requireLicense, async (req, res) => {
  try {
    const userId = req.user?.id || null
    const where = userId ? { OR: [{ usuario_id: userId }, { usuario_id: null }] } : {}
...
app.get('/api/bases-datos/:id', authOrSecret, requireLicense, async (req, res) => {
  try {
    ...
    const base = await prisma.bases_datos.findFirst({ where })
...
app.patch('/api/bases-datos/:id', authOrSecret, requireLicense, async (req, res) => {
  try {
    ...
    const existing = await prisma.bases_datos.findFirst({ where })
    ...
    if (contenido !== undefined) {
      const parsed = parsearNumeros(contenido)
      updateData.contenido = parsed.limpio
      updateData.total = parsed.total
      updateData.duplicados = parsed.duplicados
    }

    const base = await prisma.bases_datos.update({
      where: { id },
      data: updateData
    })
...
app.delete('/api/bases-datos/:id', authOrSecret, requireLicense, async (req, res) => {
  try {
    ...
    await prisma.bases_datos.delete({ where: { id } })
    res.json({ success: true })
  } catch (e) {
    res.status(500).json({ error: e.message })
  }
...
cron.schedule('*/5 * * * *', async () => {
  try {
    // 🛑 Si el tier es Starter, no ejecutar cron
    ...
    // 🧹 RECOVERY: campañas atascadas en 'processing' por más de 30 min
    const thirtyMinsAgo = new Date(Date.now() - 30 * 60 * 1000)
    const stuck = await prisma.scheduled_campaigns.findMany({
      where: {
        status: 'processing',
    ...
    for (const s of stuck) {
      console.log(`🧹 Recuperando campaña atascada: ${s.campaign_id}`)
    ...
    for (const sched of pending) {
      try {
        ...
        if (!campaign || campaign.status !== 'pending') {
          console.log(`⏭️ Campaña ${sched.campaign_id} no está en pending, saltando`)
          continue
        }
        ...
        if (lineasSeleccionadas.length === 0) {
          console.log(`❌ No hay líneas conectadas para campaña ${campaign.id}`)
          await prisma.scheduled_campaigns.update({
            where: { id: sched.id },
            data: { status: 'failed' }
          })
        ...
        // 🔥 EJECUTAR CON TIMEOUT DE 15 MINUTOS
        ...
        console.error(`❌ Error procesando scheduled ${sched.id}:`, loopErr)
        await prisma.scheduled_campaigns.update({
          where: { id: sched.id },
          data: { status: 'failed' }
        }).catch(() => {})
